...
export function TechnicianSchedule() {
  ...
  const { data: workOrders, isLoading } = useQuery({
    queryKey: ['technician-schedule', currentDate.getMonth(), currentDate.getFullYear()],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Zeitplan</h1>
        <p className="text-gray-600">Übersicht über Ihre geplanten Termine</p>
      </div>
      ...
      <div className="card p-6">
        ...
        <div className="grid grid-cols-7 gap-px bg-gray-200 rounded-lg overflow-hidden">
          ...
          {days.map((day, index) => {
            if (!day) {
              return <div key={index} className="bg-white p-3 min-h-[120px]"></div>
            }
              ...
              >
                ...
                <div className="space-y-1">
                  {dayWorkOrders.slice(0, 2).map((wo: any) => (
                    <div
                      key={wo.id}
                      className={`text-xs p-1 rounded truncate ${getStatusColor(wo.status)}`}
                      title={`${wo.properties.name} - ${getServiceText(wo.service)}`}
                    >
                      {wo.properties.name}
                    </div>
                  ))}
                  {dayWorkOrders.length > 2 && (
                    <div className="text-xs text-gray-500">
                      +{dayWorkOrders.length - 2} weitere
                    </div>
                  )}
                </div>
              </div>
